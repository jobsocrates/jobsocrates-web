...
규칙:
- 정치 관련 기사는 완전히 제외 (정치인 언급, 정당, 선거 — 해당 기사는 목록에서 빼세요)
- 어려운 용어는 괄호 안에 쉽게 설명
- 답은 주지 말 것. 생각의 방향만 제시
- 친근하고 담백한 말투
- 각 섹션은 충분히 풍부하게 써주세요 (너무 짧으면 안 됩니다)

출력 형식 (반드시 아래 구조를 정확히 지키세요):

TITLE: [오늘 기사들을 관통하는 핵심 주제, 15자 이내, 취준생이 공감할 표현]

각 기사를 아래 형식으로 작성하세요. 기사 사이는 반드시 --- 로 구분하세요:

📰 [원래 기사 제목 그대로]

무슨 일이냐면:
(이 기사가 무슨 내용인지 3~5줄로 충분히 설명. 배경 맥락 포함. 처음 듣는 사람도 이해할 수 있게.)

🔍 이걸 왜 봐야 하냐면:
(이 이슈가 취업·산업·사회와 어떻게 연결되는지. 왜 취준생 입장에서 알아두면 좋은지. 2~3줄.)

💬 스스로 물어봐:
(정답 없는 질문 1~2개. 본인 경험·관점·가치관과 연결해서 생각해보게 만드는 것. 면접에서 이 주제가 나왔을 때 자기 생각을 말할 수 있는지 점검하는 용도.)
