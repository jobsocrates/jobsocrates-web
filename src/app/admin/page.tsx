...
export default function AdminPage() {
  ...
  return (
    <div style={{ background: BG, minHeight: "100vh", color: "rgba(255,255,255,0.88)", fontFamily: "inherit" }}>
                ...
                <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
                  {detailLoading ? (
                    ...
                    <div key={item.id} style={{ marginBottom: 36 }}>
                      ...
                      {/* 소넷 원본 vs GPT 최종본 비교 — 채팅 위에 먼저 표시 */}
                      {item.revisions?.length > 0 && item.revisions[0].content && (
                        <div style={{ marginBottom: 16, padding: "16px", borderRadius: 12, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)" }}>
      ...
      {/* ─── BOARD ─── */}
      {tab === "board" && (() => {
        const tree = boardCatTree;
        const allNames = tree.flatMap(n => n.type === "item" ? [n.name, ...(n.children || [])] : []);
        function bCount(cat: string) {
          if (cat === "전체") return boardPosts.length;
          const node = tree.find(n => n.type === "item" && n.name === cat);
          if (node && node.type === "item" && node.children?.length) {
            return boardPosts.filter(p => [cat, ...node.children!].includes(p.category)).length;
          }
        ...
        function bFilter(cat: string) {
          ...
          const node = tree.find(n => n.type === "item" && n.name === cat);
          if (node && node.type === "item" && node.children?.length) {
            return boardPosts.filter(p => [cat, ...node.children!].includes(p.category));
          }
        ...
        <div className="admin-board-wrap" style={{ display: "flex", height: "calc(100vh - 56px)" }}>
          ...
          <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {/* 모바일 카테고리 셀렉트 */}
            <div className="admin-board-cat-select" style={{ display: "none", padding: "10px 16px", gap: 8, borderBottom: "1px solid rgba(255,255,255,0.07)", alignItems: "center" }}>
